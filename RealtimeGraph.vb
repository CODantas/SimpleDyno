...
''' <summary>
''' Modernized replacement for SimpleDynoSubMultiYTimeGraph (Fase 3 of the UI modernization
''' plan). Same SimpleDynoSubForm contract, same tick/axis geometry and, most importantly, the
''' exact same incremental "sweep" drawing technique as the original: DrawToBuffer only draws the
''' newest line segment on every timer tick without clearing, and the buffer is only fully
''' cleared/redrawn (grid+axes) by Redraw() when the sweep wraps around. That technique is a
''' deliberate optimization (avoids repainting the whole trace every tick) and is left untouched.
''' Visual additions are confined to Redraw()'s one-time-per-sweep drawing: dark theme colors,
''' faint background grid lines, and a distinct accent color per Y-axis/series (previously all
''' four series defaulted to the same green). No physical calculation lives in this class.
''' </summary>
Public Class RealtimeGraph
    Inherits SimpleDynoSubForm

    'Resource manager for translated (resx satellite) strings.
    Private ReadOnly resources As New System.ComponentModel.ComponentResourceManager(GetType(RealtimeGraph))
    ...
    Private Sub ApplyDefaultTheme()
        BackClr = ColorPalette.Background
        AxisClr = ColorPalette.TextSecondary
        AxisBrush.Color = AxisClr
        AxisPen.Color = ColorPalette.GridLines
        X_AxisFont = TypographyManager.UiFont(X_AxisFont.Size)
        Y_AxisFont = TypographyManager.UiFont(Y_AxisFont.Size)

        'One distinct accent per Y-axis/series - previously all four defaulted to the same green.
        Dim SeriesColors() As Color = {ColorPalette.AccentRpm, ColorPalette.AccentInfo, ColorPalette.AccentWarning, ColorPalette.AccentDanger}
        For Count As Integer = 1 To 4
            Y_DataClr(Count) = SeriesColors(Count - 1)
    ...
    'This method deliberately keeps the original incremental "sweep" technique unchanged: it only
    'draws the newest line segment on top of whatever is already in the buffer, without clearing.
    'Only Redraw() (called once per sweep wrap, not on every tick) may clear/repaint the background.
    ...
    Private Sub Redraw()
        ...
        With Grafx.Graphics
            ...
            'Faint background grid, drawn once per sweep wrap (never inside the per-tick path above).
            Using gridPen As New Pen(Color.FromArgb(110, ColorPalette.GridLines))
                For Tickcount = 1 To NumberOfMajorTicks
                    .DrawLine(gridPen, MajorXTickInner(Tickcount).X, myDataRectangle.Top, MajorXTickInner(Tickcount).X, myDataRectangle.Bottom)
                Next
                For Tickcount = 1 To NumberOfMajorTicks
                    .DrawLine(gridPen, myDataRectangle.Left, MajorYTickInner(1, Tickcount).Y, myDataRectangle.Right, MajorYTickInner(1, Tickcount).Y)
                Next
            End Using
